...
class CROW(VAE):
    ...
    def __init__(self, params):
        if not ".pkl" in params.mod1:
            super(CROW, self).__init__(
                dist.Normal,  # prior
                dist.Normal,  # likelihood
                dist.Normal,  # posterior
                Enc2(params.latent_dim, params.num_hidden_layers),
            ...
            super(CROW, self).__init__(
                ...
                dist.Normal,  # likelihood
                dist.Normal,  # posterior
                Enc(params.latent_dim, params.num_hidden_layers, params.data_dim1),
                Dec(params.latent_dim, params.num_hidden_layers, params.data_dim1),
                params
            )
        grad = {'requires_grad': params.learn_prior}
        self._pz_params = nn.ParameterList([
    ...
    def generate(self, runPath, epoch):
        N, K = 36, 1
        samples = super(CROW, self).generate(N, K).cpu().squeeze()
        # wrangle things so they come out tiled
    ...
    def reconstruct(self, data, runPath, epoch):
        recons_mat = super(CROW, self).reconstruct(data[:8]).squeeze()
        o_l = []
    ...
    def analyse(self, data, runPath, epoch):
        zemb, zsl, kls_df = super(CROW, self).analyse(data, K=10)
        labels = ['Prior', self.modelName.lower()]
        plot_embeddings(zemb, zsl, labels, '{}/emb_umap_{:03d}.png'.format(runPath, epoch))
        plot_kls_df(kls_df, '{}/kl_distance_{:03d}.png'.format(runPath, epoch))
